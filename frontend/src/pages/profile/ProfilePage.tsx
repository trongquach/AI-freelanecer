...
function ExpertReviews({ userId }: { userId: number }) {
      ...
      {isEditing ? (
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            {/* Reviews Section - EXPERT only */}
            {user?.role === 'EXPERT' && (profile?.userId || user?.id) && (
              <ExpertReviews userId={(profile?.userId ?? user?.id) as number} />
            )}
          </div>
        </div>
      )}
