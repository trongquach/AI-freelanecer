    ...
    <div className="py-2">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="section-subtitle">
            {data ? `${data.totalElements?.toLocaleString()} dịch vụ AI` : 'Đang tải...'}
          </p>
        </div>
        {isAuthenticated && isExpert() && (
          <Link to="/services/new" className="btn-gradient btn-md shrink-0">
            <Plus className="w-4 h-4" /> Tạo dịch vụ
          </Link>
        )}
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <div className="relative flex-1">
          ...
          <input className="input pl-10" placeholder="Tìm dịch vụ AI..."
      ...
      {showFilters && (
        <div className="card p-4 mb-6 grid sm:grid-cols-2 lg:grid-cols-4 gap-4 animate-slide-up">
          <div>
            <label className="block text-xs text-slate-400 mb-1">Giá tối thiểu ($)</label>
            <input type="number" className="input" placeholder="0"
              defaultValue={minPrice} onChange={e => setParam('minPrice', e.target.value || undefined)} />
          </div>
          <div>
            <label className="block text-xs text-slate-400 mb-1">Giá tối đa ($)</label>
          ...
          <div>
            <label className="block text-xs text-slate-400 mb-1">Giao hàng tối đa (ngày)</label>
          ...
          <div>
            <label className="block text-xs text-slate-400 mb-1 flex items-center gap-1">
              <Star className="w-3 h-3 text-warning-500" /> Rating tối thiểu
            </label>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 text-danger-500">Lỗi khi tải dịch vụ. Thử lại sau.</div>
      ) : !data?.content?.length ? (
        <div className="text-center py-20">
          <div className="text-6xl mb-4">🤖</div>
          <p className="text-slate-400 text-lg">Chưa có dịch vụ nào</p>
        </div>
      ) : (
        <>
          ...
          {data.totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-10">
              <button onClick={() => setParam('page', page - 1)} disabled={data.first} className="btn-secondary btn-sm disabled:opacity-40">← Trước</button>
              <span className="text-sm text-slate-400 px-3">Trang {page + 1} / {data.totalPages}</span>
              <button onClick={() => setParam('page', page + 1)} disabled={data.last} className="btn-secondary btn-sm disabled:opacity-40">Tiếp →</button>
            </div>
          )}
        </>
      )}
