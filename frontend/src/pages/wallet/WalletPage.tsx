...
export default function WalletPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <h1 className="section-title">Ví của tôi</h1>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div className="card p-6 border-primary-800">
          ...
          <p className="text-4xl font-bold text-white">${(wallet.balance - wallet.lockedAmount).toFixed(2)}</p>
        </div>
        <div className="card p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <p className="text-sm text-slate-400">Đang khóa (escrow)</p>
          </div>
      ...
      <div className="flex gap-3">
        <button className="btn-gradient btn-md flex-1"><ArrowDownLeft className="w-4 h-4" /> Nạp tiền</button>
        <button className="btn-secondary btn-md flex-1"><ArrowUpRight className="w-4 h-4" /> Rút tiền</button>
      </div>

      {/* Stats */}
      <div className="card p-6 grid sm:grid-cols-2 gap-4">
        <div>
          <p className="text-xs text-slate-500 mb-1">Tổng đã nạp</p>
          <p className="text-xl font-bold text-success-400">${wallet.totalDeposited.toFixed(2)}</p>
        </div>
        <div>
          <p className="text-xs text-slate-500 mb-1">Tổng đã giải ngân</p>
