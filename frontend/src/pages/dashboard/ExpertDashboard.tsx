...
export default function ExpertDashboard() {
  ...
  const stats = [
    { label: 'Dịch vụ', value: myServices?.totalElements ?? 0, icon: ShoppingBag, color: 'text-primary-400' },
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="section-subtitle">Quản lý dịch vụ và đề xuất dự án</p>
        </div>
        <div className="flex gap-2 shrink-0">
          <Link to="/jobs" className="btn-secondary btn-md">Tìm việc</Link>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-white">Dịch vụ của tôi</h2>
        ...
        {isLoading ? (
          ...
          <div className="card p-12 text-center">
            ...
            <Link to="/services/new" className="btn-gradient btn-md">
              <Plus className="w-4 h-4" /> Tạo dịch vụ đầu tiên
            </Link>
          </div>
        ) : (
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <Link to="/wallet" className="card-hover p-5 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Xem số dư và giao dịch</p>
          </div>
        </Link>
        <Link to="/profile" className="card-hover p-5 flex items-center gap-4">
          ...
          <div>
            <p className="font-semibold text-white">Hồ sơ chuyên gia</p>
            <p className="text-sm text-slate-500">Cập nhật kỹ năng</p>
          </div>
        </Link>
      </div>
    </div>
