  ...
  if (isSubmitSuccessful) return (
    <div className="min-h-screen bg-surface-950 flex items-center justify-center p-6">
      <div className="text-center max-w-md">
        ...
        <h1 className="text-2xl font-bold text-white mb-2">Kiểm tra email của bạn</h1>
  ...
  return (
    <div className="min-h-screen bg-surface-950 flex items-center justify-center p-6">
      <div className="w-full max-w-md">
        <Link to="/login" className="inline-flex items-center gap-2 text-slate-400 hover:text-white mb-8 text-sm"><ArrowLeft className="w-4 h-4" />Quay lại đăng nhập</Link>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <button type="submit" disabled={isSubmitting} className="btn-primary btn-lg w-full">
            {isSubmitting ? <><LoadingSpinner size="sm" /> Đang gửi...</> : 'Gửi link đặt lại'}
          </button>
        </form>
      </div>
    </div>
